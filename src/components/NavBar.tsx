...
const NavBar = () => {
  ...
  const showNavbar = () => {
    if (navRef.current) {
      // Check if navRef.current exists
  ...
  return (
    <div className="bg-lightgray text-navyblue font-alata font-semibold">
      <header>
        <Link to="/">
          <img src={logo} alt="Brand Logo" className="h-11 mr-3 scale-150" />{" "}
          {/* Adjust the h-10 class to fit the image size to your navbar */}
        </Link>
        ...
        <nav ref={navRef}>
          ...
          {currentUser ? (
            <>
              ...
              <Link to="/search" className="hover:text-blue-800">
                search
              </Link>
              <Link to="/about" className="hover:text-blue-800">
                about
              </Link>
              <Link to="/addrestraunt" className="hover:text-blue-800">
                add business
              </Link>
              <Link to="/profile">
                <img
                  src={currentUser.photoURL ?? "public/Default_pfp.svg.png"}
                  alt="Profile"
